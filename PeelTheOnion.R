...
#' This returns a plot object mapping the data to aesthetics. However, the
#' geometry is not specified, so there is nothing to see

#' A strict specification following the grammer would force the addition of
#' layers, like so:
...
#' aid addition of layers which directly control the specifics of the layer

#' It is worthwhile to pause and ponder how each of the layer components are
#' built up here
...
#' Incorrect line plot that does not convey what we wish to explore here group =
#' 1, i.e., we are saying that all the data is from the same individual There is
#' no variation per sample

ggplot(Oxboys, aes(x = age, y = height)) +
  geom_line()

#' When we wish to summarize the data, we do not want to summarize each
#' individual but rather the entire group
...
#' Each geom has an associated stat and each stat has an associated geom. Each
#' stat produces a set of variables that can be used instead of the default
#' values. These generated variables are surrounded by .. to avod any confusion
